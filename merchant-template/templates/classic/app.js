document.addEventListener('DOMContentLoaded', function () {
  ...
  function renderCart() {
    ...
    containers.forEach(function (el) {
      if (!cart.length) {
        el.innerHTML = '<p style="color:#999;font-size:0.85rem;padding:12px 0;">购物车为空</p>';
        return;
      }
  ...
  function loadMenu() {
    ...
    fetch(getApiUrl('/api/menu'))
      ...
      .catch(function () {
        containers.forEach(function (el) { el.innerHTML = '<p style="text-align:center;color:#999;padding:40px;">菜单加载失败，请稍后重试</p>'; });
      });
  }
        ...
        return (
          '<div class="menu-item fade-in" data-cat="' + d.category + '">' +
            ...
            '<div class="menu-item-body">' +
              ...
              '<button class="btn btn-primary add-to-cart" data-id="' + d.id + '" style="margin-top:10px;padding:8px 18px;font-size:0.82rem;">加入购物车</button>' +
            '</div>' +
          '</div>'
        );
      }).join('');

document.addEventListener('DOMContentLoaded', function () {
  ...
  function renderCart() {
    ...
    containers.forEach(function (el) {
      if (!cart.length) {
        el.innerHTML = '<p style="color:#999;font-size:0.85rem;padding:12px 0;">' + __('cart_empty') + '</p>';
        return;
      }
  ...
  function loadMenu() {
    ...
    fetch(getApiUrl('/api/menu'))
      ...
      .catch(function () {
        containers.forEach(function (el) { el.innerHTML = '<p style="text-align:center;color:#999;padding:40px;">' + __('menu_load_error') + '</p>'; });
      });
  }
        ...
        return (
          '<div class="menu-item fade-in" data-cat="' + d.category + '">' +
            ...
            '<div class="menu-item-body">' +
              ...
              '<button class="btn btn-primary add-to-cart" data-id="' + d.id + '" style="margin-top:10px;padding:8px 18px;font-size:0.82rem;">' + __('cart_add') + '</button>' +
            '</div>' +
          '</div>'
        );
      }).join('');
